...
| **Persistência de Dados** | SQLite + SQLAlchemy | Abordagem orientada a objetos (ORM) para mapeamento de banco de dados. O SQLite elimina a necessidade de infraestrutura de banco de dados no MVP, e o SQLAlchemy garante portabilidade (ex: migração futura para PostgreSQL). |
